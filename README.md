...
- `equipmentPoseContractsCommit`, `equipmentPoseAnchorEpsilonWu`, `resolvedSaberCapsuleContactsFlowTarget(...)`, `resolvedGloveObbContactsBoxingTarget(...)`
...
- `advance({ timestampMs, clock, input?, lease?, interaction?, equipmentPoses? })`
...
Calling `configureContent(configuration)` preserves the legacy Play configuration and publishes `play/calibrating`. Passing the exact optional `{ purpose: "visual_test" }` configuration options instead publishes configured `idle/visual_test`, or preserves an already active `playing/visual_test` / `paused_manual/visual_test` state and timeline while atomically replacing its exact content; it never introduces calibration truth. Calling `requestStart(timestampMs)` preserves the legacy normal-Play calibration/countdown path. An exact `aerobeat/gameplay_session_start` v1 request selects either `play` or `visual_test`. Visual Test starts at song time zero without Play calibration/input acquisition, remains unranked with no public calibration identity, advances only from the authoritative audio clock, and resumes directly from manual pause. It emits no outcomes by default; only the exact per-frame `aerobeat/visual_test_interaction` v1 `production_judgement` authority admits current strict evidence and resolved equipment poses to the unchanged production evaluator, always into unranked/local-only partitions. Explicit starts are restarts: they increment the coordinator generation and clear prior run truth. Completion is terminal between-run truth: ordinary `pause()` cannot convert it to a resumable manual pause, while an explicit stopped-clock `synchronizePausedClock()` seek establishes a precise `paused_manual` timeline with `explicit_seek`. Normal Play judgements are exact v2 records with `sessionPurpose: "play"` and the authoritative `committedTimelinePositionMs`; authorized Test judgements truthfully carry `sessionPurpose: "visual_test"`; synthetic demonstration feedback remains outside gameplay and scoring.

Collider evaluation is pinned by `equipmentPoseContractsCommit` to `aerobeat-web-contracts` commit `51c2b42805f5aa008386dc8bc779cfad8542af34`. Every actively evaluated Flow/Boxing collider frame requires exact `equipmentPoses`: one deeply validated resolved pose for each wrist, mode/geometry matching the selected variant, both anchors matching current measured wrist XY with `equipmentPoseAnchorEpsilonWu` (`1e-6` WU) and zero judge-plane Z within that epsilon, and one shared config identity locked for the run. The config identity is part of each collider score partition and its deterministic partition key. Flow projects the contract-resolved 3D saber capsule into judge XY; Boxing projects all transformed glove OBB corners and SAT-tests the exact convex hull against the target rectangle. No fixed-size or GLB/model-bounds fallback participates in scoring.
...
- Flow Collider evidence is current-generation only: measured frames plus the input service's `provenance: "frozen"` re-publications of the last measured frame during a calibrated mid-run tracking freeze (the 0.0.60 F4 anchor-freeze, which keeps the session `playing` and scoring live). A measured sample is fresh only while age is strict `<150ms`; a previous/current segment may span inclusive `<=150ms`. A frozen sample is EXEMPT from the `<150ms` freshness age check (its held timestamp deliberately ages past it) and is scored at the current song position, but every other validation — exact anchor name, valid + confidence, finite coordinates, calibration/timestamp equality — is identical. Finite wrist coordinates may remain off-grid; exact resolved capsule/OBB overlap decides equipment contact, while nose/body-grid semantics retain their normalized bounds. A frozen frame's per-tick identity is `(calibrationId, frozenTickId)` (a strictly increasing `frozenTickId` is a NEW frame that re-baselines the sweep and re-evaluates candidates); because a frozen frame repeats one held source frame and one measurement timestamp, the previous/current swept segment is never continuous, so only point contact can score off a frozen frame and an event can never hit twice (the `judgedIds` guard). Samples otherwise reject stale/future evidence, duplicate or rollback frames, non-finite coordinates/invalid confidence, source or calibration changes, and larger gaps; a rejected sample cannot bridge history. Each wrist is independent: one fresh valid wrist can score its owned notes and detonate bombs even when the other wrist is invalid. One sweep can resolve every genuinely intersected exact-same-center chord member, but a staggered member requires a later measured contact. Candidate ordering is deterministic and independent of source array order. Manual, tracking, lease, stopped-clock, countdown-clock, and audio-rollback pauses sever private wrist/nose continuity before recovery; each anchor's first later fresh valid frame independently seeds new history without point or swept contact; only its next continuous sample may evaluate.
...
Snapshot `hazardContact` exposes bounded presentation-only state derived from the coordinator's continuous `occupiedObstacleIds` tracking: `{active:boolean, sinceMs:number|null, releasedAtMs:number|null}`. `active` is `true` while the nose is inside at least one flow obstacle collider. `sinceMs` is the timeline ms when the current occupation episode began (first empty→non-empty transition); `releasedAtMs` is the timeline ms of the most recent non-empty→empty transition (last exit). If one obstacle is replaced by another while the set is never empty, the episode continues — `sinceMs` stays at the original episode start. The field is always `{active:false, sinceMs:null, releasedAtMs:null}` for non-`flow_colliders_v1` rulesets, non-play purposes, and after pause/reset/seek/run-end/destroy. No coordinates, confidences, or landmark data are exposed. Boxing squat/weave nose-obstacle evaluation uses a separate instantaneous checkpoint path that does not share `occupiedObstacleIds` tracking; `hazardContact` is flow-only.
