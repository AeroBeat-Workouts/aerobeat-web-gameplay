...
- Actual content-runtime envelopes are consumed directly: identity/timing stay on the resolved envelope while target, action, checkpoint, and lineage fields come from its immutable `authoredBeat`. Canonical obstacle/arc/burst envelopes require a monotonic `endTimestampMs` from `0..86,400,000` consistent with the authored beat interval; authored beats cannot own or shadow this resolved-envelope field. Legacy direct flattened interval records without `authoredBeat` may omit `endTimestampMs` and remain instantaneous ignored-compatible inputs; when a direct caller supplies an end timestamp, the same 24-hour bound and monotonicity apply.
