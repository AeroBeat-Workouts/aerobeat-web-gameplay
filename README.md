...
`applyFutureContent()` is accepted only while paused. Judged, past, and assembly-declared active events remain authoritative; only non-duplicate future lineage is replaced. Each accepted event object captures immutable package/content-generation, chart, variant, recipe/ruleset, map/score hash, tuning-profile, and scoring-settings truth. Preserved events retain that exact generation even when a swap reuses the same variant ID, while replacements use the new generation. Every accepted generation receives an internal immutable instance key, so preserved and replacement events may share the same public event ID and lineage while each judges exactly once under its own truth. Public active/judged ID telemetry remains deduplicated; stale replacements and unrelated duplicate preserved lineage are skipped. Existing judgement and score truth is never rewritten.
