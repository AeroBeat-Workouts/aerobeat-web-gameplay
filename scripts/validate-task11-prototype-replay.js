...
function input(measured, latestEvidence, options = {}) { return { sourceIdentity: "task11-replay", calibration: { calibrationId: options.calibrationId ?? "cal-1", readiness: "countdown" }, tracking: { gameplayPaused: options.paused === true, freshCalibrationRequired: options.fresh === true }, countdownFrozen: options.paused === true, latestEvidence, straightQualifications: options.qualifications ?? [] }; }
function equipmentPoses(sample) { const configIdentity = { schema: "aerobeat/equipment_config_identity", version: 1, algorithm: "sha256", value: "b".repeat(64) }; return ["left_wrist", "right_wrist"].map((role) => { const wrist = sample.anchors.find((entry) => entry.anchor === role); return { role, mode: "flow", anchor: { x: wrist.x * 4 - 0.5, y: 2.5 - wrist.y * 3, z: 0 }, scale: 1, orientation: { x: 0, y: 0, z: 0, w: 1 }, geometryIdentity: "aerobeat/saber_capsule_v1", configIdentity }; }); }
...
for (const candidate of fixture.candidateMatrix) {
  ...
  if (candidate.mode === "flow") {
    ...
    sweep.advance({ timestampMs: 4000, clock: clock(880, true), input: input(4000, before), equipmentPoses: equipmentPoses(before) });
    const after = evidence("flow-after", 4100, []);
    after.anchors.find((entry) => entry.anchor === "left_wrist").x = 0.3;
    sweep.advance({ timestampMs: 4100, clock: clock(1120, true), input: input(4100, after), equipmentPoses: equipmentPoses(after) });
