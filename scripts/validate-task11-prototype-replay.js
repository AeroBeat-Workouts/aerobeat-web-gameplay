...
function event(candidate, eventId, centerTimestampMs, type, extra = {}) { return { schema: "aerobeat/resolved_content_event", version: 2, eventId, variantId: candidate.id, chartId: candidate.chartId ?? `chart-${candidate.id}`, centerTimestampMs, sourceEventIds: [`source-${eventId}`], type, ...extra }; }
