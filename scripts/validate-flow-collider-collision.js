...
function equipmentPosesForEvidence(sample){return ["left_wrist","right_wrist"].map((role)=>{const wrist=sample.anchors.find((entry)=>entry.anchor===role);return {role,mode:"flow",anchor:{x:wrist.x*4-.5,y:2.5-wrist.y*3,z:0},scale:1,orientation:{x:0,y:0,z:0,w:1},geometryIdentity:"aerobeat/saber_capsule_v1",configIdentity:equipmentConfigIdentity};});}
...
function send(c,wallMs,songMs,left,right,nose,sourceIdentity="camera-a",frameId=`f-${wallMs}-${songMs}`){const sample=evidence(frameId,wallMs,left,right,nose);c.advance({timestampMs:wallMs,clock:clock(songMs,true),input:input(wallMs,sample,{sourceIdentity}),equipmentPoses:equipmentPosesForEvidence(sample)});return sample;}
...
// Each wrist is evaluated independently for owned notes and either-wrist bombs while both required poses are present.
{
  for(const owner of ["left","right"]){
    ...
    const noteRun=ready([beat(`independent-${owner}`,1000,"note",{hand:owner,placement})]);const noteSample=evidence(`independent-${owner}`,1000,left,right,[3,2]);noteRun.advance({timestampMs:1000,clock:clock(1000,true),input:input(1000,noteSample),equipmentPoses:equipmentPosesForEvidence(noteSample)});assert.equal(noteRun.getJudgements()[0].result,"hit",`${owner} note scores with the other wrist away`);
    const bombRun=ready([beat(`bomb-${owner}`,1000,"bomb",{placement})]);const bombSample=evidence(`bomb-${owner}`,1000,left,right,[3,2]);bombRun.advance({timestampMs:1000,clock:clock(1000,true),input:input(1000,bombSample),equipmentPoses:equipmentPosesForEvidence(bombSample)});assert.equal(bombRun.getHazardOutcomes()[0].result,"contact",`${owner} wrist detonates with the other wrist away`);
  }
}
...
// Simultaneous note plus overlapping nose-owned walls scores first and applies one deduplicated hazard break; wrists never own walls.
{
  ...
  const freshNose=ready([wall("wall-a")]);const freshSample=evidence("fresh-nose",1000,[-.4,1],[3.4,1],[1,1]);freshNose.advance({timestampMs:1149.999,clock:clock(1149.999,true),input:input(1000,freshSample),equipmentPoses:equipmentPosesForEvidence(freshSample)});assert.equal(freshNose.getHazardOutcomes()[0].result,"contact","nose age below 150ms remains fresh");
  const staleNose=ready([wall("wall-a")]);const staleSample=evidence("stale-nose",1000,[-.4,1],[3.4,1],[1,1]);assert.throws(()=>staleNose.advance({timestampMs:1150,clock:clock(1150,true),input:input(1000,staleSample),equipmentPoses:equipmentPosesForEvidence(staleSample)}),/current valid measured wrist/u,"an exactly-150ms-old active collider frame is rejected before stale nose contact");assert.notEqual(staleNose.getHazardOutcomes()[0]?.result,"contact","rejected stale frame cannot contact the wall");
}
...
// Audio rollback severs wrist/nose continuity before resume; only a genuine post-resume segment may score.
{
  ...
  const rollbackFrame=evidence("rollback-clock",800,[-.5,1],[3,1],[0,1]);c.advance({timestampMs:949,clock:clock(800,true),input:input(800,rollbackFrame),equipmentPoses:equipmentPosesForEvidence(rollbackFrame)});assert.deepEqual([c.getSnapshot().session.state,c.getSnapshot().session.pauseReason],["paused_manual","audio_clock_rollback"]);
...
// Recovery baselines are independent per anchor; unavailable peers remain seed-only until their own first valid sample.
{
  ...
  const leftOnly=invalidateWrist(evidence("left-only-seed",1000,[1,1],[3.4,1],[1,1]),"nose");c.advance({timestampMs:1000,clock:clock(1000,true),input:input(1000,leftOnly),equipmentPoses:equipmentPosesForEvidence(leftOnly)});assert.equal(c.getJudgements().length,0);
  const peersSeed=evidence("peers-seed",1050,[1.2,1],[3.4,1],[1,1]);c.advance({timestampMs:1050,clock:clock(1050,true),input:input(1050,peersSeed),equipmentPoses:equipmentPosesForEvidence(peersSeed)});assert.deepEqual(c.getJudgements().map((entry)=>entry.eventId),["asymmetric-left"],"left evaluates while newly available nose seeds and the right wrist remains away");assert.equal(c.getHazardOutcomes().length,0);
...
// Lease loss, countdown-clock violation, calibration replacement, and tracking recovery each require a new baseline.
{
  ...
  const calibrationRun=ready([beat("calibration-recovery",1000,"note",{hand:"left",placement:5})]);send(calibrationRun,900,900,[-.5,1],[3,1],[3,2]);const changed=withCalibration(evidence("new-calibration-seed",1000,[1,1],[3,1],[3,2]),"cal-2");calibrationRun.advance({timestampMs:1000,clock:clock(1000,true),input:input(1000,changed,{calibrationId:"cal-2"}),equipmentPoses:equipmentPosesForEvidence(changed)});assert.equal(calibrationRun.getJudgements().length,0);const changedNext=withCalibration(evidence("new-calibration-next",1050,[1.2,1],[3,1],[3,2]),"cal-2");calibrationRun.advance({timestampMs:1050,clock:clock(1050,true),input:input(1050,changedNext,{calibrationId:"cal-2"}),equipmentPoses:equipmentPosesForEvidence(changedNext)});assert.equal(calibrationRun.getJudgements()[0].result,"hit");

  const trackingRun=ready([beat("tracking-recovery",1000,"note",{hand:"left",placement:5})]);send(trackingRun,900,900,[-.5,1],[3,1],[3,2]);const trackingLoss=evidence("tracking-loss",900,[-.5,1],[3,1],[3,2]);trackingRun.advance({timestampMs:950,clock:clock(900,true),input:input(950,trackingLoss,{ready:false}),equipmentPoses:equipmentPosesForEvidence(trackingLoss)});assert.equal(trackingRun.getSnapshot().session.state,"paused_tracking");const recovered=withCalibration(evidence("tracking-calibration",960,[1,1],[3,1],[3,2]),"cal-2");trackingRun.advance({timestampMs:960,clock:clock(900,false),input:input(960,recovered,{calibrationId:"cal-2"})});trackingRun.advance({timestampMs:961,clock:clock(900,false)});trackingRun.advance({timestampMs:962,clock:clock(900,false)});trackingRun.advance({timestampMs:963,clock:clock(900,false)});assert.equal(trackingRun.getSnapshot().session.state,"playing");const trackingFirst=withCalibration(evidence("tracking-first",1000,[1,1],[3,1],[3,2]),"cal-2");trackingRun.advance({timestampMs:1000,clock:clock(1000,true),input:input(1000,trackingFirst,{calibrationId:"cal-2"}),equipmentPoses:equipmentPosesForEvidence(trackingFirst)});assert.equal(trackingRun.getJudgements().length,0);const trackingSecond=withCalibration(evidence("tracking-second",1050,[1.2,1],[3,1],[3,2]),"cal-2");trackingRun.advance({timestampMs:1050,clock:clock(1050,true),input:input(1050,trackingSecond,{calibrationId:"cal-2"}),equipmentPoses:equipmentPosesForEvidence(trackingSecond)});assert.equal(trackingRun.getJudgements()[0].result,"hit");
}
...
{
  for(const kind of ["duplicate","rollback","source"]){const c=ready([beat(kind,1000,"note",{hand:"left",placement:5})]);send(c,900,900,[-.5,1],[3,1],[3,2],"camera-a","baseline");if(kind==="duplicate")send(c,1000,1000,[2,1],[3,1],[3,2],"camera-a","baseline");else if(kind==="rollback"){const rollback=evidence("rollback",899,[2,1],[3,1],[3,2]);c.advance({timestampMs:1000,clock:clock(1000,true),input:input(899,rollback),equipmentPoses:equipmentPosesForEvidence(rollback)});}else send(c,1000,1000,[2,1],[3,1],[3,2],"camera-b","changed");assert.equal(c.getJudgements().length,0,kind);}
...
// byte-identical identity/monotonicity behavior.
{
  ...
  const sendFrozen=(c,songMs,heldFrameId,heldTs,tick,nose)=>{const sample=frozenEvidence(heldFrameId,heldTs,tick,nose);c.advance({timestampMs:songMs,clock:clock(songMs,true),input:input(songMs,sample),equipmentPoses:equipmentPosesForEvidence(sample)});};
